...
from dialog import graphDialogBox
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self):
        ...
        if not self._mySerialUSB.error:
            ...
            self._graphToolButtonSelected = ['1', '2']
    ...
    def _connections(self):
        ...
        self._myTabSensors.sensorUpdateGraphicSignal.connect(self._update_graphic)
    ...
    def _setup_user_interface(self):
        self.setWindowTitle('Jardinero Fiel V2.12')
        self.showMaximized()
    ...
    def _setup_graphic_toolbar(self):
        _celsiusAction = QtGui.QAction(QtGui.QIcon('images/graphicToolBar/celsius.jpeg'), 'Temperaturas', self)
        _celsiusAction.triggered.connect(self._toolbutton_celsius_action)
        _humidityAction = QtGui.QAction(QtGui.QIcon('images/graphicToolBar/humidity.png'), 'Humedad', self)
        _humidityAction.triggered.connect(self._toolbutton_humidity_action)
        _phAction = QtGui.QAction(QtGui.QIcon('images/graphicToolBar/ph.png'), 'PH', self)
        _phAction.triggered.connect(self._toolbutton_ph_action)
        _ecAction = QtGui.QAction(QtGui.QIcon('images/graphicToolBar/ec.jpg'), 'EC', self)
        _ecAction.triggered.connect(self._toolbutton_ec_action)
        _daysAction = QtGui.QAction(QtGui.QIcon('images/graphicToolBar/days.png'), 'Dias a visualizar', self)
        _daysAction.triggered.connect(self._toolbutton_x_axis_range_action)

        self._graphicToolBar.addAction(_celsiusAction)
        self._graphicToolBar.addAction(_humidityAction)
        self._graphicToolBar.addAction(_phAction)
        self._graphicToolBar.addAction(_ecAction)
        self._graphicToolBar.addSeparator()
        self._graphicToolBar.addAction(_daysAction)
    ...
    def _toolbutton_celsius_action(self):
        self._graphToolButtonSelected = ['1', '2']
        self._update_graphic()

    def _toolbutton_humidity_action(self):
        self._graphToolButtonSelected = ['3']
        self._update_graphic()

    def _toolbutton_ph_action(self):
        self._graphToolButtonSelected = ['4']
        self._update_graphic()

    def _toolbutton_ec_action(self):
        self._graphToolButtonSelected = ['5']
        self._update_graphic()

    def _toolbutton_x_axis_range_action(self):
        _value, _ok = graphDialogBox.XAxisRangeDialogBox.get_data(self._myTabGraphics.get_x_axis_range())
        if _ok:
            self._myTabGraphics.set_x_axis_range(_value)
            self._update_graphic()
    ...
    def _update_graphic(self):
        self._myTabGraphics.update_graphic(self._graphToolButtonSelected)
